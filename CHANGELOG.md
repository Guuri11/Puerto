...
- Add `harbor generate domain <Name>` — scaffolds domain layer only (model, errors, repository trait, use cases, Object Mother) and adds entity to `harbor.toml`
- Add `harbor generate application <Name>` — scaffolds application layer only (use case impls) for an entity already in `harbor.toml`
- Add `harbor generate repository <Name>` — scaffolds infrastructure layer only (InMemory or Pg repository, inferred from `harbor.toml`)
- Add `harbor generate presentation <Name>` — scaffolds presentation layer only (routes, dto, responses, error_mapper) and regenerates `bootstrap.rs`
- Object Mother test factory (`{Entity}Mother`) is now generated by `harbor generate domain` and `harbor generate scaffold` — provides `random()`, `random_vec(n)`, builder chain, and `build_props()` helpers
...
- Harbor root `Makefile` updated with ANSI colours, `format/fix`, `audit`, `audit/fix` targets
...
- `AGENTS.md` (harbor root): CLI commands table updated with four new `harbor generate` commands
...
- `harbor generate scaffold` now infers `--db` from `harbor.toml` (`project.db = true`) — no need to pass the flag when the project already uses SQLx
- Interactive `harbor new` demo prompt: shows a real project preview (`my-app`) before asking for the actual project name
- Bump to v0.5.0
