...
Two distinct workflows exist: one for developing Harbor itself, one for developing projects generated by Harbor.

---

## 1. Harbor CLI Development (Spec-Driven)

Harbor is an AI-friendly framework. Its development follows **Spec-Driven Development**: specs live in `PLAN.md` as structured phase entries. Tests are written from the spec, not from reading code. Documentation is updated only when behavior changes.
...
- Command signature (`harbor generate use-case <Entity> <action>`)
...
Tests for Harbor CLI live in `crates/cli/src/main.rs` as `#[test]` functions.
...
- `crates/cli/src/harbor_toml.rs` — harbor.toml I/O
...
For DDD entities inside projects generated by Harbor.
...
Run `harbor generate bootstrap` from project root if bootstrap is out of sync. Otherwise it was already regenerated by `harbor generate scaffold`.
...
| `crates/cli/src/harbor_toml.rs` | harbor.toml read/write/add_entity |
